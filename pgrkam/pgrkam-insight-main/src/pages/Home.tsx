...
// Public videos placed in /public (served from root path)
const testimonialVideos = ['/video_testimonial1.mp4', '/video_testimonial2.mp4', '/video_testimonial3.mp4'];
const testimonialPosters = [testimonial1, testimonial2, testimonial3];
...
const Home = () => {
  ...
  const duplicatedVideoIndexes = [
    ...testimonialVideos.map((_, i) => i),
    ...testimonialVideos.map((_, i) => i),
    ...testimonialVideos.map((_, i) => i),
  ];
  ...
  useEffect(() => {
    ...
    const originalSetLength = testimonialVideos.length;
  ...
  }, [currentTestimonialIndex, testimonialVideos.length]);
        ...
        <section className="py-12 px-6 bg-background">
          <div className="container mx-auto">
            ...
            <div className="overflow-hidden relative">
              ...
              >
                {duplicatedVideoIndexes.map((vidIdx, index) => (
                  <Card key={index} className="flex-shrink-0 w-80 mr-6 overflow-hidden">
                    <div className="aspect-video bg-black">
                      <video 
                        controls 
                        preload="metadata" 
                        poster={testimonialPosters[vidIdx]} 
                        className="w-full h-full"
                      >
                        <source src={testimonialVideos[vidIdx]} type="video/mp4" />
                        {/* Fallback text */}
                        Your browser does not support the video tag.
                      </video>
                    </div>
                    <CardContent className="p-4">
                      <p className={`text-sm ${fontSizeClasses[fontSize]}`}>{t.successStory} {index % testimonialVideos.length + 1}</p>
                    </CardContent>
                  </Card>
                ))}
              </div>
