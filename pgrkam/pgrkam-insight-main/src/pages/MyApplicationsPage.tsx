                ...
                >
                  <CardContent className="p-6">
                    <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-6">
                      ...
                      <div className="lg:ml-6 flex-shrink-0 flex items-center gap-3">
                        ...
                        {app.status !== 'Withdrawn' && app.status !== 'Hired' && (
                          <Button
                            ...
                            onClick={async () => {
                              try {
                                const resp = await fetch(`http://localhost:5000/api/jobs/withdraw/${app._id}`, {
                                  method: 'POST',
                                  headers: {
                                    'Content-Type': 'application/json',
                                    'x-auth-token': token!,
                                ...
                                if (!resp.ok) {
                                  ...
                                  throw new Error(err.msg || 'Failed to withdraw');
                                }
                                const updated = await resp.json();
                                setApplications(prev => prev.map(a => a._id === app._id ? { ...a, status: 'Withdrawn' } : a));
                              } catch (e) {
