...
const translations = {
  en: {
    welcome: "Hello! I'm the PGRKAM (Punjab Ghar Ghar Rozgar and Karobar Mission) Assistant. I can help you find jobs, training programs, and career guidance in English, Punjabi, and Hindi. How can I assist you today?",
    welcomeUser: (username: string) => `Hello, ${username}! I'm the PGRKAM (Punjab Ghar Ghar Rozgar and Karobar Mission) Assistant. How can I assist you today?`, // Personalized welcome
  ...
  hi: {
    welcome: "नमस्ते! मैं PGRKAM (Punjab Ghar Ghar Rozgar and Karobar Mission) सहायक हूं। मैं आपको अंग्रेजी, पंजाबी और हिंदी में नौकरी, प्रशिक्षण कार्यक्रम और करियर मार्गदर्शन खोजने में मदद कर सकता/सकती हूं। आज मैं आपकी कैसे सहायता कर सकता/सकती हूं?",
    welcomeUser: (username: string) => `नमस्ते, ${username}! मैं PGRKAM (Punjab Ghar Ghar Rozgar and Karobar Mission) सहायक हूं। आज मैं आपकी कैसे सहायता कर सकता/सकती हूं?`, // Personalized welcome
  ...
  pa: {
    welcome: "ਸਤ ਸ੍ਰੀ ਅਕਾਲ! ਮੈਂ PGRKAM (Punjab Ghar Ghar Rozgar and Karobar Mission) ਸਹਾਇਕ ਹਾਂ। ਮੈਂ ਤੁਹਾਨੂੰ ਅੰਗਰੇਜ਼ੀ, ਪੰਜਾਬੀ ਅਤੇ ਹਿੰਦੀ ਵਿੱਚ ਨੌਕਰੀਆਂ, ਸਿਖਲਾਈ ਪ੍ਰੋਗਰਾਮ ਅਤੇ ਕਰੀਅਰ ਮਾਰਗਦਰਸ਼ਨ ਲੱਭਣ ਵਿੱਚ ਮਦਦ ਕਰ ਸਕਦਾ/ਸਕਦੀ ਹਾਂ। ਅੱਜ ਮੈਂ ਤੁਹਾਡੀ ਕਿਵੇਂ ਮਦਦ ਕਰ ਸਕਦਾ/ਸਕਦੀ ਹਾਂ?",
    welcomeUser: (username: string) => `ਸਤ ਸ੍ਰੀ ਅਕਾਲ, ${username}! ਮੈਂ PGRKAM (Punjab Ghar Ghar Rozgar and Karobar Mission) ਸਹਾਇਕ ਹਾਂ। ਅੱਜ ਮੈਂ ਤੁਹਾਡੀ ਕਿਵੇਂ ਮਦਦ ਕਰ ਸਕਦਾ/ਸਕਦੀ ਹਾਂ?`, // Personalized welcome
...
export const ChatBot = () => {
  ...
  useEffect(() => {
    // Load available voices for TTS and keep them updated
  ...
  useEffect(() => {
    // On language change, cancel any ongoing speech and update recognition language
    if ('speechSynthesis' in window) {
  ...
  const getPreferredVoice = (lang: 'en' | 'hi' | 'pa') => {
    const langPrefix = lang === 'pa' ? 'pa' : lang === 'hi' ? 'hi' : 'en';
    // Prefer exact region matches (e.g., pa-IN), then any lang-prefix match
    ...
    // Additional heuristic for Punjabi: some browsers expose Punjabi voices with names but neutral langs
    if (lang === 'pa') {
      const nameMatch = voices.find(v => (v.name || '').toLowerCase().includes('punjabi') || (v.name || '').toLowerCase().includes('gurmukhi'));
  ...
  const speakMessage = (text: string, lang: 'en' | 'hi' | 'pa') => {
    if ('speechSynthesis' in window) {
      ...
      let effectiveLang: 'en-US' | 'hi-IN' | 'pa-IN' = lang === 'pa' ? 'pa-IN' : lang === 'hi' ? 'hi-IN' : 'en-US';
      let v = getPreferredVoice(lang);
      // Fallback: if Punjabi voice not available, use Hindi voice for better intelligibility
      if (lang === 'pa' && !v) {
        v = getPreferredVoice('hi');
        effectiveLang = 'hi-IN';
        console.warn('Punjabi TTS voice not found. Falling back to Hindi voice.');
      }
        ...
        if (selectedJobFromList) {
          ...
          addBotMessage(
            translations[language].jobDetails(
              ...
              formatSalaryRange(selectedJobFromList.salary),
